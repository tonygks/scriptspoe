...
~LControl::
{
	...
	While (GetKeyState("LControl", "P") == 1)
	{
		...
		Loop, 10
		{
			...
			If (Clipboard <> clip_parsed) and (Clipboard <> t_clip)
			{
...
Class Filter_ {
	...
	Scoring()
	{
		...
		For i, element in this.Params
		{
			...
			If (element == "CraftAbsSpellDamage")
				t_CraftFlag := True
			
			
			IsVar := "Is" . element
			;msgbox, % element " CraftFlag: "t_CraftFlag " AffixType: " Item.Get(this.AffixType[i]) " FlagAffixa: " Item.Get(IsVar) " CraftValue: " (this.ValueCraft[i]) " Corrupt: " Item.IsCorrupted 
			...
			If (t_value >= this.ValueTarget[i])
			{
				...
				If (element == "CraftPhysDPS")
					If (Item.CraftPhysDPS > Item.PhysDPS)
						t_craftFlag := True
				If (element == "CraftElemDPS")
...
CalcSpellDPS()
{
	...
	t_Suffixes := Item.Suffixes
	t_Multi := False
	Item.AbsSpellDamage := Item.SpellDamage + Item.LocalElem
	...
	Item.CraftAbsSpellDamage := Item.SpellDamage + Item.LocalElem + t_CraftLocalElem + t_CraftSpellDamage
