			...
			If (t_ClassType == element)
			{
				...
				If (this.PhysDPS[i])
				{
					If (Item.CraftPhysDPSHi > this.PhysDPS[i]) 
					{
						...
						{
							TT_PhysDPS := Round(Item.CraftPhysDPSLo) . "-" . Round(Item.CraftPhysDPSHi) . "  [CRAFT]" . TT_PhysCraftMods
							TT_PhysMultiDPS := Round(Item.MultiPhysDPSLo) . "-" . Round(Item.MultiPhysDPSHi) . "  [CRAFT]" . TT_PhysMultiMods
					...
					{
						TT_PhysDPS := Item.PhysDPS
						TT_PhysMultiDPS := T_PhysMultiDPS := Round(Item.MultiPhysDPSLo) . "-" . Round(Item.MultiPhysDPSHi) . "  [CRAFT]" . TT_PhysMultiMods
					}
				}
				TT_PhysDPS := "`nPhysDPS:	" . TT_PhysDPS . "`nMultiPhysDPS:	" . TT_PhysMultiDPS
...
CalcPhysDPS()
{
	...
	If (Item.GripType <> "1h" or Item.GripType <> "2h")
		return
