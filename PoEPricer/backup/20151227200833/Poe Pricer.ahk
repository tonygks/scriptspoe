			...
			If (t_ClassType == element)
			{
				...
				If (this.PhysDPS[i])
				{
					If (Item.CraftPhysDPSHi > this.PhysDPS[i]) 
					{
						...
						{
							TT_PhysCraftDPS := "`nPhysDPS:	" . Round(Item.CraftPhysDpsLo) . "-" . Round(Item.CraftPhysDpsHi) . "  " . TT_PhysCraftMods
							TT_PhysMultiDPS := "`nPhysDPS:	" . Round(Item.MultiPhysDpsLo) . "-" . Round(Item.MultiPhysDpsHi) . "  " . TT_PhysMultiMods
							TT_MultiAPS := "`nAPS:	" . Item.MultiAPSLo . "-" . Item.MultiAPSHi
							TT_CraftAPS := "`nAPS:	" . Item.CraftAPSLo . "-" . Item.CraftAPSHi
							t_CraftFlag := True
						}
						Item.Success := True
						TT_PhysDPS := "`nCraft:" . TT_PhysCraftDPS . "`nMultiCraft:" . TT_PhysMultiDPS 
					}
				...
				TT_PhysDPS := "`nPhysDPS:	" . Item.PhysDPS . "`nAPS:		" . Item.APS . TT_PhysDPS
				
				
				TT_Result := "`n--------------------------------------" . TT_PhysDPS . TT_ElemDPS . TT_Crit . "`n--------------------------------------"
...
CalcPhysDPS()
{
	...
	Item.APSLo := Item.BaseAPS*(100+Item.IASLo)/100
	Item.APSLo := Item.BaseAPS*(100+Item.IASHi)/100
	Item.CraftedAPSLo := Item.BaseAPS * (100 + Item.IAS + t_CraftIASLo)/100
	Item.CraftedAPSHi := Item.BaseAPS * (100 + Item.IAS + t_CraftIASHi)/100
