	...
	Scoring()
	{
		...
		For i, element in this.Params
		{
			...
			If (t_value >= this.ValueTarget[i])
			{
				...
				If t_craftElement
				{
					TT_Result := TT_Result . "`n" . element . ":" . pad . Round(t_value) . "[CRAFT +" . this.ValueCraft[i] . "]"
					TT_ResultExt := TT_ResultExt . "`n" . element . ":" . pad . Round(t_value) . "  [" . Round(t_Score) . "]  [CRAFT +" . this.ValueCraft[i] . "]"
				}
				else
				{
